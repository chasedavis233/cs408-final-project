// In-browser demo data store for BiteRec (used before AWS hookup)
...
const seed = {
  ...
  places: [
    ...
    },
    {
      ...
      favorite: false,
      image:
        "https://images.unsplash.com/photo-1514511542647-8f6e120f0b2b?w=1200&q=80&auto=format&fit=crop",
    },
  ],
};
...
// Convenience accessors for demo data
...
// Toggle favorite flag for a place
...
// Update status field for a place
...
// Card used on the home grid
export function restCard(p) {
  const heart = p.favorite ? "❤️" : "🤍";
  return `
  <a class="card rest-card" href="place.html?id=${encodeURIComponent(
    p.id
  )}" data-id="${p.id}">
...
// Row-style card used in list views
export function entryRow(p) {
  return `
  <a class="entry card rest-card" href="place.html?id=${encodeURIComponent(
    p.id
  )}" data-id="${p.id}">
    ...
    <div class="entry-body">
      ...
      <div class="row muted">
        ...
        ${
          p.price
            ? `<span class="sep"></span><span>Total: $${p.price.toFixed(
                2
              )}</span>`
            : ""
        }
      ...
      ${
        p.notes
          ? `<div class="row small muted"><strong>What I had:</strong> <span class="chip">${esc(
              p.notes
            )}</span></div>`
          : ""
      }
    </div>
    <div class="entry-rating" aria-label="Rating">
